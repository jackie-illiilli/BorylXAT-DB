...
1. **`boron_ccl2.db`**：ASE SQLite 数据库，适合通过 ASE 高效存储 `Atoms` 对象、检索键值对元数据并追踪反应映射关系。
2. **`boron_ccl_dataset2.parquet`**：Parquet 数据集，以扁平化二维表的形式存储坐标、原子序数及描述符，更适合 Pandas、PyArrow 和机器学习任务。
...
## 1. ASE SQLite 数据库（`boron_ccl2.db`）
...
## 2. Parquet 数据集（`boron_ccl_dataset2.parquet`）
