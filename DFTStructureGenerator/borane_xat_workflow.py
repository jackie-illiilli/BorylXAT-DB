...
def complete_target(
    ...
    cl_df=None,
    name="other_multi",
    output_dir="Data/Iteration_Data",
    output_csv=None,
    bn_csv_path="Data/csvs/reactants_B_N.csv",
    cl_csv_path="Data/csvs/reactants_Cl.csv",
    hartree_to_kcal=627.5,
):
    """Complete a TS target table from B/N/Cl indices.

    The input table must contain at least ``B_Index``, ``N_Index``, and
    ``Cl_Index``. The returned table adds reactant SMILES, reactive atom ids,
    conformer ids, reaction free energy, and reactant complex energy. When
    ``output_csv`` is not provided, the table is written to
    ``{output_dir}/{name}.csv`` for compatibility with the historical notebook
    workflow.
    """
    if bn_df is None:
        bn_df = pd.read_csv(bn_csv_path)
    if cl_df is None:
    ...
    for _, row in data_csv.iterrows():
        ...
        bn_matches = bn_df[
            (bn_df["B_Index"] == b_index) &
            (bn_df["N_Index"] == n_index)
        ]
        cl_matches = cl_df[cl_df["Index"] == cl_index]
        if bn_matches.empty:
            raise KeyError(f"Missing B/N pair: B_Index={b_index}, N_Index={n_index}")
        if cl_matches.empty:
            raise KeyError(f"Missing chloride substrate: Cl_Index={cl_index}")

        bn_row = bn_matches.iloc[0]
        cl_row = cl_matches.iloc[0]
        records.append({
            "B_smiles": bn_row["B_smiles"],
            "B_Index": b_index,
            "B_Atomid": int(bn_row["B_Atomid"]),
            "N_smiles": bn_row["N_smiles"],
            "N_Index": n_index,
            "N_Atomid": int(bn_row["N_Atomid"]),
            "Cl_smiles": cl_row["Smiles"],
            "Cl_Index": cl_index,
            "Cl_Atomid": int(cl_row["Atomid"]),
            "B_N_Cl_conf": int(bn_row["conf_idxs_p"]),
            "Cl_r_conf": int(cl_row["conf_idxs_r"]),
            "deltaG_react": hartree_to_kcal * (
                bn_row["deltaG_react"] + cl_row["deltaG_react"]
            ),
            "G_energy": bn_row["G_energy_r"] + cl_row["G_energy_r"],
        })

    completed_df = pd.DataFrame(records)
    if output_csv is None and name is not None:
        output_csv = os.path.join(output_dir, f"{name}.csv")
    if output_csv is not None:
        output_folder = os.path.dirname(output_csv)
        if output_folder:
            os.makedirs(output_folder, exist_ok=True)
        completed_df.to_csv(output_csv, index=False)
    return completed_df
...
# disabled by repo-wide static call scan: generate_ts_structure_simple
...
# disabled by repo-wide static call scan: detail_irc
...
# disabled by repo-wide static call scan: check_irc
...
def prepare_irc_jobs(
    ...
    need_irc_files = []
    for b_id, n_id, cl_id in tqdm(available_ts_files):
